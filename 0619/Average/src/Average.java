...
public class Average extends Configured implements Tool {
	...
	public static class Map extends MapReduceBase
		...
		public void map(LongWritable key, Text value,
			...
			while (stk.hasMoreTokens()) {
				// Task I, transform a line into <Key, IntPair> as you wish
				try {
    				output.collect(
                        new Text(stk.nextToken()),
                        new IntPair(Integer.parseInt(stk.nextToken()), 1));
	...
	public static class Combine extends MapReduceBase
		...
		public void reduce(Text key, Iterator<IntPair> values,
				OutputCollector<Text, IntPair> output, Reporter reporter)
				throws IOException {
			// Task II, collect the part of Iterator<IntPair>,
			// 			and then store them into <Key, IntPair>
            int sum = 0;
            int n = 0;
            while (values.hasNext()) {
                IntPair value = values.next();
                sum += value.getFirst();
                n += value.getSecond();
            }
            output.collect(key, new IntPair(sum, n));
        }
	}
	...
	public static class Reduce extends MapReduceBase
		...
		public void reduce(Text key, Iterator<IntPair> values,
				...
				throws IOException {
			// Task I, transform <Key, Iterator<IntPair>> into <Key, Double>
			double sum = 0.0;
            int n = 0;
            while (values.hasNext()) {
                IntPair value = values.next();
            ...
            output.collect(key, new DoubleWritable(sum / n));
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		job.setJobName("Average - B03902036");
